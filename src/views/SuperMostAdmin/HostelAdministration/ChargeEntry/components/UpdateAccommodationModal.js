...
const DURATION_OPTIONS = ['Monthly', '1 Month', '3 Months', '6 Months', '12 Months', '24 Months'];

const UpdateAccommodationModal = ({ open, onClose, onConfirm, initialData = null, loading = false }) => {
  const [amount, setAmount] = useState('₹ 3,000');
  const [duration, setDuration] = useState('Monthly');
  const [notes, setNotes] = useState('Updated accommodation charge');
    ...
    if (open && initialData) {
      const rawNum =
        typeof initialData.amount === 'number'
          ? initialData.amount
          : Number(String(initialData.accommodationCharges || initialData.amount || 3000).replace(/[^0-9.]/g, '')) || 3000;

      setAmount(`₹ ${rawNum.toLocaleString('en-IN')}`);
      setDuration(initialData.duration || 'Monthly');
      setNotes(initialData.notes && initialData.notes !== '-' ? initialData.notes : 'Updated accommodation charge');
      setAmountError('');
    }
  }, [open, initialData]);
  ...
  const handleUpdate = () => {
    ...
    if (onConfirm) {
      onConfirm({
        id: initialData?.id || initialData?._id || initialData?.accommodationId,
        amount: numAmount,
        duration: duration || 'Monthly',
        notes: notes.trim() || 'Updated accommodation charge'
      });
    }
  };
      ...
      <DialogContent sx={{ p: 0, overflow: 'visible' }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <Box>
            ...
            >
              Update the charge amount, duration, and notes for this accommodation entry.
            </Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <TextField
              ...
              placeholder="e.g. Updated accommodation charge"
              disabled={loading}
              sx={{
                bgcolor: '#ffffff',
                borderRadius: '8px',
