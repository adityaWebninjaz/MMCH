...
// Initial mock dataset for Fixed Buildings (flat-rate ₹750)
export const INITIAL_FIXED_DATA = [
  {
    id: 'FIX-REC-001',
    buildingName: 'Ward 1B (Hostel Block A)',
    buildingNo: 'Building 1B',
    roomNo: '101',
    employeeName: 'Dr. Sudhanshu Verma',
    employeeId: 'PMCH-1001',
  ...
  {
    id: 'FIX-REC-003',
    buildingName: 'Ward 2A (Resident Block)',
    buildingNo: 'Building 2A',
    roomNo: '201',
    employeeName: 'Dr. Alok Nath',
    employeeId: 'PMCH-1003',
    ratePlan: 'Fixed Flat Rate',
    monthlyRate: '750',
    charge: '750',
    status: 'Confirmed',
  ...
  },
  {
    ...
    buildingNo: 'Building 2A',
    roomNo: '202',
    employeeName: 'Dr. Ritu Saxena',
    employeeId: 'PMCH-1004',
    ratePlan: 'Fixed Flat Rate',
    monthlyRate: '750',
    charge: '750',
    status: 'Confirmed',
  ...
  {
    id: 'FIX-REC-005',
    buildingName: 'Ward 3C (Nursing Quarters)',
    buildingNo: 'Building 3C',
    roomNo: '301',
    employeeName: 'Sister Mary Joseph',
    employeeId: 'PMCH-1005',
    ratePlan: 'Fixed Flat Rate',
    monthlyRate: '750',
    charge: '750',
    status: 'Confirmed',
  ...
  },
  {
    ...
    buildingNo: 'Building 3C',
    roomNo: '302',
    employeeName: 'Staff Nurse Anjali',
    employeeId: 'PMCH-1006',
    ratePlan: 'Fixed Flat Rate',
    monthlyRate: '750',
    charge: '750',
    status: 'Confirmed',
...
const METERED_STORAGE_KEY = 'MMCH_METER_READINGS_METERED_DATA';
const FIXED_STORAGE_KEY = 'MMCH_METER_READINGS_FIXED_DATA';
...
export const saveMeterReading = async (readingData) => {
  ...
  return {
    success: true,
    data: newEntry,
    message: 'Charge / Reading entry saved successfully'
  };
};
...
export const exportMeterReadingsExcel = ({ data = [], tab = 'metered' }) => {
  ...
  if (tab === 'metered') {
    ...
    rows = data.map((item) => [
      ...
      `"${item.roomNo || ''}"`,
      `"${item.employeeName || ''}"`,
      `"${item.ratePlan || 'Fixed Flat Rate'}"`,
      `"INR ${item.monthlyRate || '750'}"`,
      `"INR ${item.charge || '750'}"`,
    ...
    .map((item) =>
      tab === 'metered'
        ...
        : `<tr>
            ...
            <td>${item.roomNo || '-'}</td>
            <td>${item.employeeName || '-'}</td>
            <td>${item.ratePlan || 'Fixed Flat Rate'}</td>
            <td>₹ ${item.monthlyRate || '750'}</td>
            <td style="font-weight: 700;">₹ ${item.charge || '750'}</td>
